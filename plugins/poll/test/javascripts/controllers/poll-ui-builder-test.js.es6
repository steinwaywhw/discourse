...
test("number pollOutput", function() {
  ...
  equal(controller.get("pollOutput"), "[poll name=test type=number min=1 max=20 step=2]\n[/poll]", "it should return the right output");

  controller.set("publicPoll", true);

  equal(controller.get("pollOutput"), "[poll name=test type=number min=1 max=20 step=2 public=true]\n[/poll]", "it should return the right output");
});
